...
async function makePdf(file) {
	...
	// extract the media
	const media = Object.values(epub.manifest).filter(
		(meta) => !/(xml|html)$/.test(meta["media-type"]),
	);
	...
	for (const meta of media) {
		if (meta.href.endsWith("css"))
			cssLinks.push(`<link rel="stylesheet" href="${meta.href}">`);
	}
...
`;
	// final html build
	const cleanBook = clean
		? removeSwearWords(unwantedChars(book))
		: unwantedChars(book);
	const html = `
  <html lang="en">
    <head>

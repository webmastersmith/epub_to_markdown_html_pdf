...
export function removeSwearWords(text) {
	...
	const regex = new RegExp(`\\b(?:${swearWords.join("|")})\\b`, "iu");
...
export async function writeFile(meta, epub, buildPath, fontSize = 1) {
	...
	if (meta.href.endsWith("css")) {
		const fontSizeFix = buf
			.toString()
			.replace(/font-size: ?([0-9.]+)(\w+)/g, (_, c1, c2) => {
				return `font-size: ${(Number.parseFloat(c1) * 1000 * (fontSize * 1000)) / 1000000}${c2}`;
			});
...
export function unwantedChars(txt) {
	return (
		txt
			...
			.replace(/—|⸺|―/g, "-")
			.replaceAll("•", "*")
			// biome-ignore lint/suspicious/noControlCharactersInRegex: replace special characters from text.
...
export function removeNonAscii(
	str,
	fileName = false,
	directory = false,
	link = false,
	...
	// links
	if (link)
		return txt.replaceAll(/[^-A-Za-z0-9]/g, "-").replaceAll(/_{2,}/g, "_");
	// directory
	if (directory)
		return txt.replaceAll(/[^A-Za-z0-9_]/g, "_").replaceAll(/_{2,}/g, "_");
